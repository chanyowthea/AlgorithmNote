...
namespace ICSharpCode.SharpZipLib.Zip.Compression 
{
	...
	public class InflaterHuffmanTree
	{
        ...
        {
			BuildTree(codeLengths, index);
		}
        #endregion

        static Dictionary<int, short[]> m_HuffmanTreeCache = new Dictionary<int, short[]>();
		...
		public void BuildTree(byte[] codeLengths, int index ,bool use_same_mem = false)
		{
			...
			*/
            if(index>=0)
            {
                if(m_HuffmanTreeCache.TryGetValue(index,out tree) == false
                    || tree == null
                    || tree.Length < treeSize)
                {
                    tree = new short[treeSize];
                    m_HuffmanTreeCache[index] = tree;
                }
                else
                {
