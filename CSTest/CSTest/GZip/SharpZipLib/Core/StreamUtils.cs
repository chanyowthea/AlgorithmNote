...
namespace ICSharpCode.SharpZipLib.Core
{
    ...
    public sealed class StreamUtils
	{
        ...
        static public void CopyFromGzipInputStream(GZip.GZipInputStream source, Stream destination, byte[] buffer, bool enable_optimize = false)
        {
            if (source == null)
            {
                throw new ArgumentNullException("source");
            ...
            while (copying)
            {
                ...
                if(enable_optimize)
                {
                    bytesRead = source.OptimizeRead(buffer, 0, buffer.Length);
                }
                else
                {
